...
<html>
	<head>
		...
		<script type="text/javascript">
			$(function() {
				$("#mytable").DataTable({
					...
					columnDefs: [{
						...
						data: function (row, type, val, meta) {
							// 在序号位置，row就代表JSON中list所对应的一个位置的数据对象
							var detailBtn = "<a href='javascript:reviewDetail(" + row.purchaseId + ");'>查看详情</a>"
							// 对于撤回申请的按钮来说，必须该申请为当前登录用户所能够操作的
							// 获得当前登录用户信息
							var userId = ${sessionScope.user.userId};
							var applyBackBtn = "";
							if (userId == row.applicant.userId) {
								applyBackBtn = "<a href='javascript:applyBackPurchase(" + row.purchaseId + ");' class='btn btn-danger btn-xs'><i class=''></i>撤回申请</a>";							}
							var reviewBtn = "";
			...
			function applyBackPurchase(purchaseId) {
				var url = "purchase/applyBack/" + purchaseId;
				var width = "600px";
				var height = "300px";
				var titleHTML = "<i class='fa fa-hand-stop-o'></i>&nbsp;撤回采购申请";
				// 对于子页面要想获得父页面，JavaScript提供了另外一个对象parent
				parent.showModal(url, width, height, titleHTML);
			}
			function reviewDetail(purchaseId) {
				var url = "purchase/reviewDetail/" + purchaseId;
				var width = "720px";
				var height = "400px";
				var titleHTML = "<i class='fa fa-sort-amount-desc'></i>&nbsp;采购审批详情";
				// 对于子页面要想获得父页面，JavaScript提供了另外一个对象parent
				parent.showModal(url, width, height, titleHTML);
			}
		</script>
